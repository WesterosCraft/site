...
export const fetchLocationsByRegion = async (region: string): Promise<Location[]> => {
  const query = {
    query: `{
      Locations(
        limit: 300
        where: { projectDetails__information__region: { equals: ${camelize(region)} } }
      ) {
        docs {
          id
          locationName
  ...
  const locations = await fetcher(`http://localhost:3000/api/graphql`, 'POST', query)
...
export const fetchLocationBySlug = async (slug: string): Promise<Location> => {
  ...
  const location = await fetcher(`http://localhost:3000/api/graphql`, 'POST', query)
