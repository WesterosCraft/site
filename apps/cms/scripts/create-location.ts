...
const uploadImage = async (ref: string, fileName: string) => {
...
function mergeListItems(arr: any[] | undefined) {
  if (!arr) {
    return []
  }
  const mergedArray = []
...
const sanityRichTextToPayloadRichText = async (sRichText: any, id: string) => {
  const newRT = [] as any[]
...
const uploadLocationData = async (data: any) => {
  ...
  try {
    if (data?.bannerImage && Object.keys(data?.bannerImage).length) {
      bannerData = await uploadImage(data.bannerImage.asset._ref, `${data._id}-banner`)
    }
    ...
    if (data?.additionalImages?.images && data.additionalImages.images?.length) {
      ...
      for (const [i, image] of data?.additionalImages.images.entries()) {
        const result = await uploadImage(image.asset._ref, `${data._id}-${i + 1}`)
